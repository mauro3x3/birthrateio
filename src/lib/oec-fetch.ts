...
export function buildOecTradeUrl(
  oecId: string,
  flow: TradeFlow,
  limit: number,
  token?: string | null,
...
function parseTradeJson(
  ...
  return {
    ...
    source: json.annotations?.source_name
      ...
      : "OEC / BACI",
    sourceUrl:
      json.annotations?.dataset_link ?? "https://oec.world/en/resources/api",
  };
}

/** Fetch latest HS4 product mix for a country (works in browser — OEC allows CORS *). */
export async function fetchCountryTrade(
  iso3: string,
  flow: TradeFlow,
  limit = 45,
  token?: string | null,
  ...
  try {
    const res = await fetch(buildOecTradeUrl(oecId, flow, limit, token), {
    ...
    return parseTradeJson(json, flow, oecId);
  } catch {
    return null;
  }
}

export async function fetchCountryTradePair(iso3: string): Promise<{
  exports: CountryTrade | null;
  imports: CountryTrade | null;
}> {
  const [exports, imports] = await Promise.all([
    fetchCountryTrade(iso3, "export"),
    fetchCountryTrade(iso3, "import"),
  ]);
